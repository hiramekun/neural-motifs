...
for img_i in trange(len(val)):
    ...
    gt_indices = np.arange(
        gt_entry['gt_boxes'].shape[0])  # np.random.choice(gt_entry['gt_boxes'].shape[0], 20)
    ...
    rel_order = np.argsort(-rel_scores[:, 1:].max(1) * obj_scores[pred_rel_inds[:, 0]] * obj_scores[
        pred_rel_inds[:, 1]])

    pred_entry = {
        'pred_boxes': pred_boxes,
        'pred_classes': gt_entry['gt_classes'][gt_indices],
        # 1+np.random.choice(150, pred_boxes.shape[0], replace=True),
    ...
    #########################
    predicate_scores_theirs = np.zeros((pred_boxes.shape[0], pred_boxes.shape[0], 51),
                                       dtype=np.float64)
    ...
    roidb_entry = {
        'max_overlaps': np.concatenate(
            (np.ones(gt_entry['gt_boxes'].shape[0]), np.zeros(pred_entry['pred_boxes'].shape[0])),
            0),
        'boxes': np.concatenate((gt_entry['gt_boxes'], pred_entry['pred_boxes']), 0),
        'gt_classes': gt_entry['gt_classes'],
        'gt_relations': gt_entry['gt_relations'],
    }
    eval_relation_recall(sg_entry_orig_format, roidb_entry, result_dict_theirs, 'sgdet',
                         iou_thresh=0.5)
